...
//    Rot3D1.RemoveFirstItem;  // Ingen bra idé, kommer att tömma grafen :-)
   end;
  Rot3D1.MakeVisible;
...
begin
 // List index out of bounds här nånstans, bara när det finns rader
...
if lb_editlog.Items.Count>0 then
  ...
  //    tmpstr:=IntToStr(q)+': '+String(ADODataset2.Recordset.Fields.Item['trackpoint_id'].Value);
     ...
     begin
       if lb_editlog.Items[j][1]='*' then     // borde det inte vara "i" istället för "0" ???
      ...
      begin
       ...
       if lb_editlog.Items[0][1]='D' then    //     change from 0 to i
        begin
         ...
         sql2:='DELETE FROM trackedit WHERE trackpoint_id='+tmpstr+' AND track_id='+IntToStr(current_track_id);  // track_id behövs!!!
         ADOCommand1.CommandText:=sql2;
         ADODataset2.Recordset:=ADOCommand1.Execute;
        end;
      end; // for i:=
